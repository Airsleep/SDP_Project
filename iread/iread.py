...
parser.add_argument(
    'bam_file', help="A coordinate-sorted BAM file resulting from read alignment. Samtools can be used for sorting.")
parser.add_argument(
    'intron_file', help="a file listing introns in bed format.")
parser.add_argument('-o', '--output_folder',
                    help="a folder to output intron retention results. It will be created if not exist. And if it already exists, all data from the folde will be delted first.")
parser.add_argument('-n', '--number_of_reads',
                    help="threshold of number of intronic reads to determine an intron retention event. Default: 20")
parser.add_argument('-j', '--junction_reads',
                    help="threshold of number of exon-intron junction reads to determine an intron retention event. Default: 1")
parser.add_argument(
    '-f', '--fpkm', help="threshold of FPKM to determine an intron retention event. Default: 3")
parser.add_argument(
    '-e', '--entropy', help="threshold of entropy_score to determine an intron retention event. Range of this parameter is [0,1]. This score measures to which extent reads are uniformly distributed aross the whole region for a given intron. Default: 0.9")
...
parser.add_argument(
    '-k', '--n_cores', help="The number of CPU cores to use. Default to n-2, where n is the total number of cores available.")
...
parser.add_argument('-s', '--threads',
                    help="Number of threads to use. Default to 20.")
parser.add_argument(
    '-m', '--mem', help="memory to use (unit: G). Default to 30.")
...
# bam file 전처리, 파일 확장자가 bam인지 확인한 후 이름을 따로 저장
...
cmd_reduce = 'bam2intron ' + ' ' + bam_file + ' ' + intron_file + ' ' + output_folder + \
    ' ' + pure_file_name + ' ' + str(MAPQ) + ' '+str(threads)+' '+str(mem)+'G'
# print(cmd_reduce)
os.system(cmd_reduce)

# count intronic reads
cmd_count = 'count_intronic_reads.pl ' + output_folder + \
    ' ' + pure_file_name + ' ' + n_cores + ' ' + intron_file
...
cmd_assess = 'assess_intron.py ' + output_folder+" "+pure_file_name+" " + \
    str(number_of_reads)+" " + str(junction_reads)+" " + str(entropy) + \
    " "+str(fpkm)+" " + str(total_reads)+" " + str(bias)
